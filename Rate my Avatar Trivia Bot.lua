...
apiUrl = "https://the-trivia-api.com/api/questions?limit=1&region=US&difficulty=easy" -- if you want to make your own settings, go here (https://the-trivia-api.com/docs/) and paste the url / keep limit to 1 
getgenv().announceSkippingHastags = true -- if your booth has hashtags it will automatically skip - if you want to say in chat that is skipping, set this to true
...
getgenv().correctAnswer = "Made by Sw1ndler"
...
messageDoneFiltering.OnClientEvent:Connect(function(message)
    ...
    if player == game.Players.LocalPlayer and message == "!skip" then
        skipping = true
        timeLeft = 0
    end
        ...
        if string.match(correctAnswer:lower(), message:lower()) then
            if string.len(string.match(correctAnswer:lower(), message:lower())) >= 4 then
                ...
                if playerScore ~= false then
                    correctPlayerScore =  playerScore + 1
                    addPoints(player.Name)
                else
                    table.insert(scoreboard, {[player.Name] = 1})
                    correctPlayerScore = 1
                end
            end
        end
    end
...
while wait() and doTrivia do
    ...
    if answered == true then
        setBooth(correctPlayer .. " answered right and now has " .. correctPlayerScore .. " points", 6641087396)
        wait(3)
        answered = false
        timeLeft = 0
    end
